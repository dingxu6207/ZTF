...
def getdata(allpara):
    ...
    noisy = np.interp(x, phase, dataym) #y轴
...
for i  in range(0,hang):
   ...
   ##################MCMC#########################
    ...
    ##############################################
        ...
        dataymn = np.hstack((datay[offset:], datay[:offset])) 
        sx2,sy2,snoise2 = intertwo(phase, dataymn)
        try:
            stdx2, R_2 = calculater(sy2, resultflux)
        except:
            R_2 = 0
        print('R_2  is： '+str(R_2))
        
        
        ax.plot(phase, dataymn, '.', c='b')
        ...
        if doflag ==1 :
            ...
            stdcancha,r_2 = calculater(noisy, resultflux)
            stdnoisy = np.diff(noisy, 2).std()/np.sqrt(6)
            selectflag = stdcancha/stdnoisy
        ...
        tempalldata = []
        if modelindex == 0:
            tempalldata = [name,  RA, DEC, P, prob, mu[0]*5850, sigma[0]*5850, mu[1]*90, sigma[1]*90, mu[2], sigma[2], mu[3], sigma[3], mu[4], sigma[4], 0, 0, inputtemper, R[index], pbdic, pr1, pr2, stdflag, stdcancha, selectflag,R_2]
        if modelindex == 1:
            tempalldata = [name,  RA, DEC, P, prob, mu[0]*5850, sigma[0]*5850, mu[1]*90, sigma[1]*90, mu[2], sigma[2], mu[3], sigma[3], mu[4], sigma[4], mu[5], sigma[5], inputtemper, R[index], pbdic, pr1, pr2, stdflag, stdcancha, selectflag,R_2]
